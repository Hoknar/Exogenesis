...
/obj/machinery/mineral/stacking_unit_console/New()
	..()
	spawn(7)
		src.machine = locate(/obj/machinery/mineral/stacking_machine) in range(src, 7)
		if (machine)
			machine.CONSOLE = src
		else
			del(src)
...
/obj/machinery/mineral/stacking_machine/New()
	...
	spawn( 5 )
		for (var/dir in cardinal)
			src.input = locate(/obj/machinery/mineral/input) in range(src, 2)
			if(src.input) break
		for (var/dir in cardinal)
			src.output = locate(/obj/machinery/mineral/output) in range(src, 2)
			if(src.output) break
