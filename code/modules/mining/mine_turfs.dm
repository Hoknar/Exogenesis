...
/turf/unsimulated/mineral //wall piece
	...
	New()
		...
		spawn(1)
			var/turf/T
			if((istype(get_step(src, NORTH), /turf/unsimulated/floor)) || (istype(get_step(src, NORTH), /turf/space)))
				T = get_step(src, NORTH)
				if (T)
					T.overlays += image('icons/turf/walls.dmi', "rock_side_s")
			if((istype(get_step(src, SOUTH), /turf/unsimulated/floor)) || (istype(get_step(src, SOUTH), /turf/space)))
				T = get_step(src, SOUTH)
				if (T)
					T.overlays += image('icons/turf/walls.dmi', "rock_side_n", layer=6)
			if((istype(get_step(src, EAST), /turf/unsimulated/floor)) || (istype(get_step(src, EAST), /turf/space)))
				T = get_step(src, EAST)
				if (T)
					T.overlays += image('icons/turf/walls.dmi', "rock_side_w", layer=6)
			if((istype(get_step(src, WEST), /turf/unsimulated/floor)) || (istype(get_step(src, WEST), /turf/space)))
				T = get_step(src, WEST)
				if (T)
					T.overlays += image('icons/turf/walls.dmi', "rock_side_e", layer=6)
...
//			O.geologic_data = geologic_data
	...
	proc/GetDrilled(var/artifact_fail = 0)
		...
		var/turf/unsimulated/floor/plating/asteroid/N = ChangeTurf(/turf/unsimulated/floor/plating/asteroid)
		N.fullUpdateMineralOverlays()
...
/turf/unsimulated/floor/plating/asteroid //floor piece
	name = "Strange Sand"
	icon = 'icons/turf/floors.dmi'
	icon_state = "asteroid"
//	icon_plating = "asteroid"
...
/turf/unsimulated/floor/plating/asteroid/New()
	var/proper_name = name
	..()
	name = proper_name
	//if (prob(50))
...
/turf/unsimulated/floor/plating/asteroid/ex_act(severity)
	switch(severity)
		if(3.0)
			return
		if(2.0)
...
/turf/unsimulated/floor/plating/asteroid/attackby(obj/item/weapon/W as obj, mob/user as mob)
...
/turf/unsimulated/floor/plating/asteroid/proc/gets_dug()
	if(dug)
		return
	new/obj/item/weapon/ore/glass(src)
	new/obj/item/weapon/ore/glass(src)
...
/turf/unsimulated/floor/plating/asteroid/proc/updateMineralOverlays()
...
/turf/unsimulated/floor/plating/asteroid/proc/fullUpdateMineralOverlays()
	var/turf/unsimulated/floor/plating/asteroid/A
	if(istype(get_step(src, WEST), /turf/unsimulated/floor/plating/asteroid))
		A = get_step(src, WEST)
		A.updateMineralOverlays()
	if(istype(get_step(src, EAST), /turf/unsimulated/floor/plating/asteroid))
		A = get_step(src, EAST)
		A.updateMineralOverlays()
	if(istype(get_step(src, NORTH), /turf/unsimulated/floor/plating/asteroid))
		A = get_step(src, NORTH)
		A.updateMineralOverlays()
	if(istype(get_step(src, NORTHWEST), /turf/unsimulated/floor/plating/asteroid))
		A = get_step(src, NORTHWEST)
		A.updateMineralOverlays()
	if(istype(get_step(src, NORTHEAST), /turf/unsimulated/floor/plating/asteroid))
		A = get_step(src, NORTHEAST)
		A.updateMineralOverlays()
	if(istype(get_step(src, SOUTHWEST), /turf/unsimulated/floor/plating/asteroid))
		A = get_step(src, SOUTHWEST)
		A.updateMineralOverlays()
	if(istype(get_step(src, SOUTHEAST), /turf/unsimulated/floor/plating/asteroid))
		A = get_step(src, SOUTHEAST)
		A.updateMineralOverlays()
	if(istype(get_step(src, SOUTH), /turf/unsimulated/floor/plating/asteroid))
		A = get_step(src, SOUTH)
		A.updateMineralOverlays()
	updateMineralOverlays()
...
/turf/unsimulated/floor/plating/asteroid/Entered(atom/movable/M as mob|obj)
	..()
	if(istype(M,/mob/living/silicon/robot))
		var/mob/living/silicon/robot/R = M
		if(istype(R.module, /obj/item/weapon/robot_module/miner))
