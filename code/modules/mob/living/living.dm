...
/mob/living/verb/resist()
	...
	if(istype(src.loc,/mob/living/simple_animal/borer))
		...
		var/mob/living/simple_animal/captive_brain/H = src
...
//		CM.last_special = world.time + 100
		...
		spawn(0)
//			world << "DEBUG: [L] is starting to break out of [C]."
//			world << "DEBUG: Breakout time: [breakout_time*60*10]."
			spawn((breakout_time*60*10)) //minutes * 60seconds * 10deciseconds
//				world << "DEBUG: Breakout countdown complete."
				if(!C || !L || L.stat != 0 || L.loc != C || C.opened) //closet/user destroyed OR user dead/unconcious OR user no longer in closet OR closet opened
//					world << "DEBUG: Closet/user destroyed OR user dead/unconcious OR user no longer in closet OR closet opened."
					return

				//Perform the same set of checks as above for weld and lock status to determine if there is even still a point in 'resisting'...
				if(istype(L.loc, /obj/structure/closet/secure_closet))
//					world << "DEBUG: This is a secure closet."
					var/obj/structure/closet/secure_closet/SC = L.loc
					if(!SC.locked && !SC.welded)
						SC.open()
						return
