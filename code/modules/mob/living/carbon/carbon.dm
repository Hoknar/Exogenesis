...
//		var/datum/mind/hostmind = B.mind
//		ticker.mode.update_borer_icons_removed(hostmind) //Remove HUD
		B.ckey = ckey
		B.controlling = 0
	...
	ticker.mode.update_all_borer_icons() //update HUD

	verbs -= /mob/living/carbon/proc/release_control
	verbs -= /mob/living/carbon/proc/punish_host
	verbs -= /mob/living/carbon/proc/spawn_larvae
	verbs -= /mob/living/carbon/proc/thrall_tent
...
/mob/proc/num_thrall_tent()
	var/N = 0
	for(var/I in contents)
		if(istype(I,/mob/living/thrall_tent))
			N++
...
/mob/living/carbon/proc/thrall_tent()
	set category = "Alien"
	set name = "Thrall Tentacle (250)"
	set desc = "Inserts a Thrall Tentacle, making it easier to convert the host to a thrall."
	...
	if(B.chemicals >= 250)
		src << "\red <B>Your host's eyes roll back and they convulse as you start to insert a Thrall Tentacle.</B>"
		B.host_brain << "\red <B>You can feel a strange pressure in your brain as if something is squirming inside.</B>"
		visible_message("\red <B>[src] falls and convulses, their eyes rolling back in their head!</B>")
		B.chemicals -= 250
		src.Weaken(8)

		spawn(100)
			new /mob/living/thrall_tent(src)

			playsound(loc, 'sound/effects/squelch1.ogg', 10, 1)

			src << "\red <B> You now have [num_thrall_tent()] Thrall Tentacles in [src]!"
			B.host_brain << "\red <B>The squirming in your brain stops, but it still feels like there is something there...</B>"

	else
		src << "You do not have enough chemicals stored to spawn a Thrall Tentacle."
		return