...
/mob/living/silicon/say(var/message)
	...
	if (length(message) >= 2)
		...
		if (department_radio_keys[prefix] == "binary")
			...
			robot_talk(message)
		else if (isAI(src)&&client)
			var/mob/living/silicon/ai/U = src
			if (department_radio_keys[prefix] == "department")
				message = copytext(message, 3)
				message = trim(copytext(sanitize(message), 1, MAX_MESSAGE_LEN))
				U.holopad_talk(message)
			else if (department_radio_keys[prefix] == "Supply")
				message = copytext(message, 3)
				message = trim(copytext(sanitize(message), 1, MAX_MESSAGE_LEN))
				U.radio.talk_into(U, message, "Supply")
			else if (department_radio_keys[prefix] == "Medical")
				message = copytext(message, 3)
				message = trim(copytext(sanitize(message), 1, MAX_MESSAGE_LEN))
				U.radio.talk_into(U, message, "Medical")
			else if (department_radio_keys[prefix] == "Security")
				message = copytext(message, 3)
				message = trim(copytext(sanitize(message), 1, MAX_MESSAGE_LEN))
				U.radio.talk_into(U, message, "Security")
			else if (department_radio_keys[prefix] == "Engineering")
				message = copytext(message, 3)
				message = trim(copytext(sanitize(message), 1, MAX_MESSAGE_LEN))
				U.radio.talk_into(U, message, "Engineering")
			else if (department_radio_keys[prefix] == "Command")
				message = copytext(message, 3)
				message = trim(copytext(sanitize(message), 1, MAX_MESSAGE_LEN))
				U.radio.talk_into(U, message, "Command")
			else if (department_radio_keys[prefix] == "Science")
				message = copytext(message, 3)
				message = trim(copytext(sanitize(message), 1, MAX_MESSAGE_LEN))
				U.radio.talk_into(U, message, "Science")
			else
				return ..(message)
		else
			return ..(message)
