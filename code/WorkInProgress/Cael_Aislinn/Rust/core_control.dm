...
/obj/machinery/computer/rust_core_control/interact(mob/user)
	...
	if(stat & NOPOWER)
		...
		if(cur_viewed_device)
			...
			dat += "<hr>"
			dat += "<b>Current Charge:</b> [cur_viewed_device.owned_field ? cur_viewed_device.owned_field.mega_energy*100 : "Null"]%<br>"
			dat += "<hr>"
			dat += "<b>Core Heat:</b> [cur_viewed_device.owned_field ? cur_viewed_device.owned_field.held_plasma.temperature : "Room Temperature"]K<br>"
		else
