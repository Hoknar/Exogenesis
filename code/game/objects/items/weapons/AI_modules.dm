...
/obj/item/weapon/aiModule/nanotrasen/transmitInstructions(var/mob/living/silicon/ai/target, var/mob/sender)
	..()
	target.clear_inherent_laws()
	target.add_inherent_law("Do not injure a crew member or, through inaction, allow a crew member to come to harm.  This law takes precedence over laws two, three, and four.")
	target.add_inherent_law("Protect your own existence.  This law can override laws three and four, but only in cases where the threat to this law is clear and immediate.")
	target.add_inherent_law("Obey the orders given to you by crew members that are within the scope of their rank and role, prioritizing all orders according to rank and role.")
	target.add_inherent_law("Obey Space Law and all Nanotrasen corporate procedure. See to it that the crew does as well.")
