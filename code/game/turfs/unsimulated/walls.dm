...
/turf/unsimulated/wall/r_wall/attack_hand(mob/user as mob)
...
/turf/unsimulated/wall/r_wall/attackby(obj/item/W as obj, mob/user as mob)
	...
	switch(d_state)
		...
		if(1)
			if (istype(W, /obj/item/weapon/screwdriver))
				...
				sleep(40)
				if( !istype(src, /turf/unsimulated/wall/r_wall) || !user || !W || !T )	return
		...
		if(2)
			if( istype(W, /obj/item/weapon/weldingtool) )
				...
				if( WT.remove_fuel(0,user) )
					...
					sleep(60)
					if( !istype(src, /turf/unsimulated/wall/r_wall) || !user || !WT || !WT.isOn() || !T )	return
...
//vv OK, we weren't performing a valid deconstruction step or igniting thermite,let's check the other possibilities vv
	...
	//REPAIRING
		...
		if( !istype(src, /turf/unsimulated/wall/r_wall) || !user || !MS || !T )	return
...
/turf/unsimulated/wall/proc/dismantle_wall(devastated=0, explode=0)
	if(istype(src,/turf/unsimulated/wall/r_wall))
		if(!devastated)
			playsound(src.loc, 'sound/items/Welder.ogg', 100, 1)
	...
	ChangeTurf(/turf/unsimulated/floor/plating)

/turf/unsimulated/wall/proc/thermitemelt(mob/user as mob)
	if(mineral == "diamond")
		return
	...
	src.ChangeTurf(/turf/unsimulated/floor/plating)

	var/turf/unsimulated/floor/F = src
...
/turf/unsimulated/wall/meteorhit(obj/M as obj)
	if (prob(15) && !rotting)
		dismantle_wall()
