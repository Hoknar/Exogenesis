...
/proc/cancel_call_proc(var/mob/user)
	...
	if(emergency_shuttle.crewtrans)
		user << "Crew transfer has been called by NT. [station_name()] personel do not have the authority to recall the shuttle. If the shuttle needs to be recalled due to an extreme factor please send a communication or fax to CentCom for review. If reasoning is sound, shuttle will be recalled."
	else if(emergency_shuttle.direction != -1 && emergency_shuttle.online) //check that shuttle isn't already heading to centcomm
